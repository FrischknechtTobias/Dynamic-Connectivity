...
//states how many occupation numbers exist
...
typedef struct EulerTour {
     uint8_t level; //Level of this eulerTour

    uint32_t nedges; //Number of (active) edges in this spanning forest/ET
    ...
    uint32_t nccomp; //number of trees in double linked list of number of connected components in this forest
} EulerTour;

static inline uint8_t activeEdge(const s_node *c) {
    return c->msk & 0b01000000;
}

static inline uint8_t vertexType(const s_node *c) {
    if(!c)
        return 0;
    else
