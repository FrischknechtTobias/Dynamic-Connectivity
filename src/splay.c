...
struct s_tree *tl, uint8_t insLeft) {
 ...
 */ 
struct s_tree * join(struct s_tree *tleft, struct s_tree *tright, uint8_t useLeft){
        if(useLeft) {
            if(EMPTY_TREE(tleft)) {
 ...
 */
struct s_tree *  deleteNode(struct s_node *d, struct s_tree *t , uint8_t del) {
    tmp_tree.root =NULL;
...
uint8_t smaller(struct s_node *a, struct s_node *b) {
    splay(a);
    splay(b);
    if(a == b->sn_left)
        return 1;
...
*/
struct s_tree * getTree(struct s_node *c,uint8_t withSplay) {
    if(!c)
        return NULL;
    if(withSplay) {
        splay(c);
 ...
 **************************************************************************
 */
uint32_t get(s_node* n,int i){
    return n? (n->oN[i]):0;
} 
/**************************************************************************
 **************************************************************************
 */
void calcON(s_node *n){
    if(!n)
        return;
    int i=0;

    for(int i=0;i<n->numberOfON;i++){
        n->oN[i]=n->oNS[i]+get(n->sn_left,i)+get(n->sn_right,i);
    }
}
/**************************************************************************
 **************************************************************************
 */
/*
    The occupation number of node n with index number will be set to value
    Also starts the propagation upwards of this change. 
*/
void setONS(int value,s_node *n,int number){
 ...
 */
/*
    The occupation number of node n of the whole component with index number will be returned 
    n is splayed so is the new root, helps 
*/
int getON(s_node *n,int number){
 ...
 */
/*
    The occupation number of node n of the node  with index number will be returned 

*/
int getONn(s_node *n,int number){
